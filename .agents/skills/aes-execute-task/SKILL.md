...
1. Ground in repository and PRD context.
   ...
   - If the caller provides workflow memory paths, use the installed `aes-workflow-memory` skill before editing code.
...
4. Validate and self-review.
   ...
   - Use the installed `aes-final-verify` skill. This step is mandatory regardless of auto-commit mode — always verify before claiming completion.
   - Check the finished deliverable field by field against every contract-bearing spec artifact identified by the survey (aes-final-verify "Spec Contract Parity"), using the resolved interpretation from step 1 when sources had disagreed. A mismatch against that resolved contract fails completion — fix the deliverable and re-verify; do not reinterpret the contract to match what was built, and do not pause to ask.
