...
2. Load context.
   - Read `_prd.md`, `_techspec.md`, `_user_stories.md`, and `_tests.md` from `.aes/tasks/<name>/`.
   - Read existing ADRs from `.aes/tasks/<name>/adrs/` to understand the decision context behind requirements and design choices.
...
6. Generate task files.
   - Write `_tasks.md` as the canonical task graph manifest. It MUST start with this YAML frontmatter shape:
     ...
     # Lista de Tasks de [Nome da Feature]
     ```
...
7. Enrich each task file.
   - For each task file, check whether it already has `## Visão Geral`, `## Entregáveis`, and `## Testes` sections, plus a complete `## Contrato Visual` when it implements visible UI from a named reference. Skip enrichment only when every required and conditional section is complete.
   ...
   - Write all prose content (Visão Geral, Subtarefas, Entregáveis, Critérios de Sucesso, requirement/subtask descriptions) in Portuguese (Brazil); section headers are Portuguese too — copy them exactly as specified in `references/task-template.md`, never re-translate or vary the spelling between tasks. Frontmatter keys/values and IDs stay exactly as specified.
   - Fill ALL template sections from `references/task-template.md`. Every task file MUST contain each of the following sections — omitting any is a failure:
     ...
     - `### ADRs Relacionados`: links to relevant ADRs if any exist, or omit the subsection if none apply.
     ...
     - `## Critérios de Sucesso`: measurable outcomes including "Todo caso de teste atribuído implementado e passando".
   - When the task implements visible UI from a named visual reference, also include `## Contrato Visual` from the template. Its deliverables and success criteria MUST require the durable `eng-ui-screenshot` evidence bundle for every row; implementation-only captures are invalid evidence.
...
8. Audit the resulting task package.
   ...
   - Self-check before finishing: every PRD goal, user story, and TechSpec component has a home in at least one task, no task file is missing a required section from `references/task-template.md`, and every dependency implied by the PRD/TechSpec is recorded in `_tasks.md`'s `graph.edges`.
